...
public class TessBaseAPI
{
	...
	public class ProgressValues
	{
		 ...
		 * 
		 * The returned bounding box is in the Android coordinate system, which
		 * has the origin in the top left.
		 * 
		...
		/**
		 * Return the left bound of the word currently being recognized. Uses
		 * Tesseract coordinate system, with origin in the bottom left.
		 * 
		...
		/**
		 * Return the right bound of the word currently being recognized. Uses
		 * Tesseract coordinate system, with origin in the bottom left.
		 * 
		 * @deprecated Use {@link #getCurrentWordRect()} instead.
		 */
		...
		/**
		 * Return the top bound of the word currently being recognized. Uses
		 * Tesseract coordinate system, with origin in the bottom left.
		 * 
		 * @deprecated Use {@link #getCurrentWordRect()} instead.
		 */
		...
		/**
		 * Return the bottom bound of the word currently being recognized. Uses
		 * Tesseract coordinate system, with origin in the bottom left.
		 * 
		 * @deprecated Use {@link #getCurrentWordRect()} instead.
		 */
	 ...
	 * 
	 * @param datapath
	 *            the parent directory of tessdata ending in a forward slash
	 * @param language
	 *            an ISO 639-3 string representing the language(s)
		...
		if (ocrEngineMode != OEM_CUBE_ONLY)
		{
			for (String languageCode : language.split("\\+"))
			{
				if (!languageCode.startsWith("~"))
				{
					...
					if (!datafile.exists())
						throw new IllegalArgumentException(
								"Data file not found at " + datafile);
				}
			}
		}
	 ...
	 * <p>
	 * Supply the name of the parameter and the value as a string, just as you
	 * would in a config file.
	 * <p>
	 * Returns false if the name lookup failed.
	 * <p>
	 * Eg <code>setVariable("tessedit_char_blacklist", "xyz");</code> to ignore
	 * x, y and z.
	 * 
	 * Or <code>setVariable("classify_bln_numeric_mode", "1");</code> to set
	 * numeric-only mode.
	 * <p>
	 * setVariable may be used before init, but settings will revert to defaults
	 * on end().
	 * <p>
	 ...
	 * 
	 * @param enabled
	 *            <code>true</code> to enable debugging mode
	 */
	 ...
	 * 
	 * @param image
	 *            Leptonica pix representation of the image
	 */
	 ...
	 * <p>
	 * Caller takes ownership of the Pix and must recycle() it. May be called
	 * any time after setImage.
	 * 
	 ...
	 * <p>
	 * Can be called before or after Recognize. Note: the caller is responsible
	 * for calling recycle() on the returned Pixa.
	 * 
	 ...
	 * 
	 * @param page
	 *            is 0-based but will appear in the output as 1-based.
	 ...
	 * <p>
	 * Searches the standard places: <i>tessdata/configs,
	 * tessdata/tessconfigs</i>. Note: only non-init params will be set.
	 * 
	 * @param filename
	 *            the configuration filename, without the path
	 */
	...
	/**
	 * The recognized text is returned as coded in the same format as a UTF8 box
	 * file used in training.
	 * <p>
	 * Constructs coordinates in the original image - not just the rectangle.
	 * 
	 * @param page
	 *            a 0-based page index that will appear in the box file.
	 */
	 ...
	 *            Percent complete
	 * @param left
	 *            Left bound of word bounding box
	 * @param right
	 *            Right bound of word bounding box
	 * @param top
	 *            Top bound of word bounding box
	 * @param bottom
	 *            Bottom bound of word bounding box
	 * @param textLeft
	 *            Left bound of text bounding box
	 * @param textRight
	 *            Right bound of text bounding box
	 * @param textTop
	 *            Top bound of text bounding box
	 * @param textBottom
	 *            Bottom bound of text bounding box
	 */
	protected void onProgressValues(final int percent, final int left,
			final int right, final int top, final int bottom,
			final int textLeft, final int textRight, final int textTop,
			final int textBottom)
	{
		
		if (progressNotifier != null)
		{
			Rect wordRect = new Rect(left, textTop - top, right, textTop
					- bottom);
	 ...
	 * 
	 * @param title
	 *            a title to be used in the document metadata
	...
	/**
	 * Finishes the document and finalizes the output data. Invalid if
	 * beginDocument not yet called.
	 * 
	 * @return {@code true} on success. {@code false} on failure
	 */
	 ...
	 *            image to be used for OCR
	 * @param imageToWrite
	 *            path to image to be written into resulting document
	 * 
	 * @return {@code true} on success. {@code false} on failure
	 */
	...
	private native boolean nativeAddPageToDocument(long nativePix,
			String imagePath, long rendererPointer);
}
